...
with graph.as_default():
    ...
    #
        with tf.variable_scope('Generator'):
            ...
            Generator_affine_1  = fully_connected(inputs, 128,
                                weights_initializer=tf.random_normal_initializer(stddev=0.02),
                                # weights_regularizer=tc.layers.l2_regularizer(2.5e-5),
                                activation_fn=tf.nn.relu)
            Generator_reshape_1 = tf.reshape(Generator_affine_1, [batch_size, 1, 1, 128])
            Generator_deconv_1  = convolution2d_transpose(Generator_reshape_1, 64, [7, 7], [7, 7],
                                weights_initializer=tf.random_normal_initializer(stddev=0.02),
                                # weights_regularizer=tc.layers.l2_regularizer(2.5e-5),
                                activation_fn=tf.nn.relu)

            Generator_deconv_2  = convolution2d_transpose(Generator_deconv_1, 16, [2, 2], [2, 2],
                                weights_initializer=tf.random_normal_initializer(stddev=0.02),
                                # weights_regularizer=tc.layers.l2_regularizer(2.5e-5),
                                activation_fn=tf.nn.relu)

            Generator_deconv_3  = convolution2d_transpose(Generator_deconv_2, 1, [2, 2], [2, 2],
                                weights_initializer=tf.random_normal_initializer(stddev=0.02),
                                # weights_regularizer=tc.layers.l2_regularizer(2.5e-5),
